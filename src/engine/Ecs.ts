...
export class ECS {
    ...
    private _subscribe ( ) {
        this.eventAggregator.subscribe('onDisableSystem', (systemId: string) => {
            this._runOrPushToAfterUpdateStack(function () {
                let systemIndex = this._systems.findIndex((system: EcsSystem) => {
                    return system.id = systemId;
                });

                this._inactiveSystems.push(this._systems[systemIndex]);
        ...
        this.eventAggregator.subscribe('onEnableSystem', (systemId: string) => {
            this._runOrPushToAfterUpdateStack(function () {
                ...
                this._systems.push(this._inactiveSystems[systemIndex]);
                this._inactiveSystems.splice(systemIndex, 1);
            });
        });
    }
